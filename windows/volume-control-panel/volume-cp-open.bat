:: Simple way to open the volume control panel so I can open it using logi ring action using a batch file.
:: Some commands will only work for Windows 11 

@echo off
echo Opening the Windows sound control panel...
:: Optional commands (COPY ONLY THE )
...
start "" mmsys.cpl & REM Open the volume control panel
if errorlevel 1 (
	echo Failed to open.
	exit /b 1
)
